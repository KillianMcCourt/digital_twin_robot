...
XTrain = filterCellArray(XTrain);
XVal=filterCellArray(XVal);
YVal=categorical(createAlternatingSeries(XVal));
YTrain =categorical(createAlternatingSeries(XTrain));
...
function filteredCellArray = filterCellArray(cArray)
    numCells = numel(cArray);
    numSets = floor(numCells / 13); % Number of complete sets of 13 cells

    filteredCellArray = cell(1, 2 * numSets); % Initialize the new filtered cell array
    idx = 1; % Index for filteredCellArray

    for setIdx = 0:numSets-1
        startIdx = setIdx * 13 + 1;
        endIdx = startIdx + 12;
        currentSet = cArray(startIdx:endIdx);

        % Keep the first cell
        filteredCellArray{idx} = currentSet{1};
        idx = idx + 1;

        % Select one random cell from the remaining 12
        randomIdx = randi([10, 13]);
        filteredCellArray{idx} = currentSet{randomIdx};
        idx = idx + 1;
    end

    % Remove any empty cells (if the last set was not a complete set of 13 cells)
    filteredCellArray = filteredCellArray(~cellfun('isempty', filteredCellArray));
end


function alternatingSeries = createAlternatingSeries(XTrain)
    % Get the total number of cells in the input list XTrain
    totalLength = numel(XTrain);

    % Create an alternating series of 0s and 1s
    alternatingSeries = mod(0:totalLength-1, 2);
end