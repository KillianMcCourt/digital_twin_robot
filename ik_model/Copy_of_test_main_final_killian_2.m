...
processing_simulation(1,'training',1,'trajectory_dataset_name','dataset_jpoint_realistic_traj_300_motorerror000205','realistic_trajs',1, 'realistic_trajs_number',600 ,'selection_erreures_moteur',[ 0,6,7,8,9,21,22,23,24])
%'jvalues',1
%processing_simulation(1,'training',0,'trajectory_dataset_name','cellArray_3NewPidRealisticTraj_m1234_e00020305','realistic_trajs',1, 'realistic_trajs_number',3,'name_trained_AI_model','bi_lstm_v2traj_500_rt_motorerror_00020305_1234_1000.mat','selection_erreures_moteur',[ 0,6,7,8,9,11,12,13,14,21,22,23,24])
...
%--------------------------------------------------------------------------
    ...
    addOptional(p, 'jvalues',0, @isnumeric);
    addOptional(p, 'training',0, @isnumeric);
    addOptional(p, 'plotting',0, @isnumeric);
    ...
    addOptional(p, 'fit_to_ram', 0, @isnumeric);
    ...
    addOptional(p, 'nb_points_traj', 100, @isnumeric);
    ...
    fprintf('Il y aura plotting Y/N: %d.\n',plotting);
    assignin('base','plotting',  plotting);
    jvalues = p.Results.jvalues;
    fprintf('Il y aura jvalues Y/N: %d.\n',jvalues);
    ...
    total_number_of_gen=realistic_trajs*realistic_trajs_number+interpolations*interpolation_number+circles*circle_number+lines*line_number;
    fprintf('total_number_of_gen: %d .\n', total_number_of_gen);
    motorerrorselection = p.Results.selection_erreures_moteur;
    selection_erreures_moteur= motorerrorselection;
    ...
    fit_to_ram = p.Results.fit_to_ram;
    fprintf('Il y aura fit_to_ram Y/N: %d.\n',fit_to_ram);
    if fit_to_ram
        disp("Your generation will automatically cut into chunks of 500 trajectories in order to fit into around 16gigas of ram per set, for infromation these chunks of 500 take around 24h hours to be generated and simulated on a 12th Gen Intel(R) Core(TM) i5 - 12400f processor, this entire file for code will be run multiple times so these initialisation texts will be printed multiple times equally at each restart of  each generation of a chunk, do not be surprised by this. ")
    end
   if fit_to_ram && ~simulating 
       disp("You are not simulating but you are trying to fit to Ram, therfore your fit to ram command will be ignored as you are only training a model")
       fit_to_ram=0;
   end 
 ...
 if ~fit_to_ram || total_number_of_gen < 501
    fit_to_ram = 0;
    %Creating the various trajectory datasets for the wanted shape types
    ...
    if firsttype
        ...
        for k = 1:numberofshapes
            ...
            dataset_1=[dataset_1,five_motor_command_simulation(plotting,trajectory_motor_command,len_time_series,motorerrorselection,m1,m0,targets,model_name,mdl,base,follower,stationary_error,stationary_error_timestap,k,num_classes,jvalues)];
        end
    ...
    % %%%%%
            ...
            dataset_2=[dataset_2,five_motor_command_simulation(plotting,trajectory_motor_command,len_time_series,motorerrorselection,m1,m0,targets,model_name,mdl,base,follower, stationary_error,stationary_error_timestap,k,num_classes,jvalues)];
        end
    ...
    if thirdtype
        ...
        for k = 1:numberofshapes
            ...
            j4=shape{4}; 
            j5=shape{5};
            [x,y,z]=ForwardKinematic(j1, j2, j3, j4, j5,len_time_series, "target");
            targets=[x,y,z];
            assignin('base','targets', targets)
            ...
            dataset_3=[dataset_3,five_motor_command_simulation(plotting,trajectory_motor_command,len_time_series,motorerrorselection,m1,m0,targets,model_name,mdl,base,follower,stationary_error,stationary_error_timestap,k,num_classes,jvalues)];
        end
        ...
        %fprintf('joint1_damping: %d.\n',joint1_damping);
        fit_to_ram_dataset_genertion(joint1_damping,joint2_damping,damp_pince,mdl,base,follower,targetIDs,outputIDs,guessesIDs,guesses,trained_AI_model,name_trained_AI_model,nb_points_traj,stationary_error,stationary_error_timestap,average_smallest_motive_lenght,zero_amount,speedcap,distance_charac_robot,model_name,fit_to_ram,training,trajectory_dataset_name, circles,lines,interpolations, realistic_trajs, selection_erreures_moteur, len_time_series, circle_number, line_number, interpolation_number, realistic_trajs_number,jvalues)
    end
...
function [circlelist] = CreateCircleList(max_rayon,max_eloignement_centre)
        ...
        %pas de 0.1 choisi dans premières boucles for experimentalement
...
function [interpolated_set] = createInterpolate(numberofinterpolatedshapes,len_time_series)
    ...
    for p = 1:numberofinterpolatedshapes
        ...
        end_time_value_in_seconds= (len_time_series-1)*0.1;

        ts_x = resample(ts_x, 0:0.1:end_time_value_in_seconds);
        ts_y = resample(ts_y, 0:0.1:end_time_value_in_seconds);
        ts_z = resample(ts_z, 0:0.1:end_time_value_in_seconds);

        % ts_x = resample(ts_x, 0.1:0.1:10);
        % ts_y = resample(ts_y, 0.1:0.1:10);
        % ts_z = resample(ts_z, 0.1:0.1:10);
        % 
...
%function 11
function [output_dataset] = five_motor_command_simulation(plotting,input_motor_commands,len_time_series,motorerrorselection,m1,m0,targets,model_name,mdl,base,follower,stationary_error,stationary_error_timestap,k,num_classes,jvalues)
%_______________
...
%_______________
        ...
        joint1_ts = timeseries(j1/180*pi,0:0.1:end_time_value_in_seconds);
        assignin('base','joint1_ts', joint1_ts)
        joint2_ts = timeseries(j2/180*pi,0:0.1:end_time_value_in_seconds);
        assignin('base','joint2_ts', joint2_ts)
        joint3_ts = timeseries(j3/180*pi,0:0.1:end_time_value_in_seconds);
        assignin('base','joint3_ts', joint3_ts)
        joint4_ts = timeseries(j4/180*pi,0:0.1:end_time_value_in_seconds);
        assignin('base','joint4_ts', joint4_ts)
        joint5_ts = timeseries(j5/180*pi,0:0.1:end_time_value_in_seconds);
...
for j = motorerrorselection
    ...
    %activates when you want to get j1,j2,...,j5 values in the dataset
    if plotting
        size_out=size(output_dataset)
        size_1=size(placeholder1)
    end 
    modifier_placeholder1=reshape(placeholder1, [100, 1]);
    if plotting
    size_1=size( modifier_placeholder1)
    size_2=size(placeholder2)
    size_3=size(placeholder3)
    size_4=size(placeholder4)
    end
    if jvalues
        output_dataset = [output_dataset,modifier_placeholder1,placeholder2,placeholder3,placeholder4]; %,placeholder5];
    end
    if plotting
        disp('size(output_dataset) 1:')
        disp(size(output_dataset))
    end
    ...
    switch j
        ...
        case 5  
            ...
            assignin('base', 'error5', error5)
        case 6
            joint1_ts.Data = process_points(joint1_ts.Data,plotting);
            assignin('base', 'joint1_ts', joint1_ts)
        case 7
            joint2_ts.Data = process_points(joint2_ts.Data,plotting);
            assignin('base', 'joint2_ts', joint2_ts)
        case 8
            joint3_ts.Data = process_points(joint3_ts.Data,plotting);
            assignin('base', 'joint3_ts', joint3_ts)
        case 9
            joint4_ts.Data = process_points(joint4_ts.Data,plotting);
            assignin('base', 'joint4_ts', joint4_ts)
        case 10
            joint5_ts.Data = process_points(joint5_ts.Data,plotting);
            assignin('base', 'joint5_ts', joint5_ts)
...
time = joint1_ts.Time; % Assuming all timeseries objects have the same time vector
            ...
            [x, y, z] = ForwardKinematic(j1o, j2o, j3o, j4o, j5o,len_time_series,string(j) ); 
            if plotting


% If you still want to scatter some points with a color gradient, you can
% add it to each plot, but this example keeps it simple with just the plots.
            end
            ...
            if plotting
                disp('size(output_dataset) 3:')
                disp(size(output_dataset))
            end
            %activates if you wanna collect j1, ...j5,
            %currenlty only restriced up to j4
            if jvalues
                output_dataset=[output_dataset,j1o, j2o, j3o, j4o];
            end
            if plotting
                disp('size(output_dataset) 4:')
                disp(size(output_dataset))
            end
...
%function 12
function [x, y ,z] = ForwardKinematic(j1, j2, j3, j4, j5,len_time_series, text)
    joint1_damping = 0;
    joint2_damping = 0;
    damp_pince = 1000; 
...
plot(x, 'o-'); % Plot X data with circle markers and lines
grid on;
xlabel('Time Step');
ylabel('X Position');
...
plot(y, 'o-'); % Plot Y data with circle markers and lines
grid on;
xlabel('Time Step');
ylabel('Y Position');
...
plot(z, 'o-'); % Plot Z data with circle markers and lines
grid on;
xlabel('Time Step');
ylabel('Z Position');
...
function updated_j1 = process_points(j1,plotting)
    % Initialize pointsList
    pointsList = zeros(length(j1), 1);
    disp("numel pointslist")
    disp(numel(pointsList))

    % Track if at least one block is set to zero
    zeroBlockExists = false;

    % Loop through each block of 0.2*numel(j1) points
    for i = 1:floor(length(j1)/(floor(0.2*numel(j1))))
        start_index = floor((i-1)*0.2*numel(j1) + 1);
        end_index = floor(i*0.2*numel(j1));
    ...
    if ~zeroBlockExists
        blockIndex = randi(floor(length(j1)/0.2*numel(j1)));
        start_index = (blockIndex-1)*0.2*numel(j1) + 1;
        end_index = blockIndex*0.2*numel(j1);
        pointsList(start_index:end_index) = 0;
    end
    ...
    if length(pointsList) > lastZerosIndices(end) + 0.2*numel(j1) && all(pointsList(lastZerosIndices(end) + 1:lastZerosIndices(end) + 0.2*numel(j1)) == 1)
        if plotting
        ...
        endValue = j1(lastZerosIndices(end) + 0.2*numel(j1));
        interpolatedValues = linspace(startValue, endValue, 0.2*numel(j1));
        j1(lastZerosIndices(end) + 1:lastZerosIndices(end) + 0.2*numel(j1)) = interpolatedValues;
    end

    % Iterate over pointsList to set j1(i) to j1(i-1) where necessary
    %disp(pointsList)
    pointsList = [ones(0.2*numel(j1),1); zeros(0.8*numel(j1),1)];
    ...
    updated_j1 = j1;
    disp("numel updated j1")
    disp(numel(updated_j1))
end


%function 14
function updated_j1 = process_points_capped_speed(j1, cap, time_scale)
%sample values : cap = 2, time_scale  = 0.1
    for i = 2:numel(j1)
...
function updated_trajectory = extend_trajectory(originalPoints, scaleFactor)
    ...
    % Iterate through each block of 0.2*numel(j1) points
    for i = 1:numBlocks
        start_index = (i-1)*blockSize + 1;
        end_index = i*blockSize;
...
%function 18
function [] = fit_to_ram_dataset_genertion( joint1_damping,joint2_damping,damp_pince,mdl,base,follower,targetIDs,outputIDs,guessesIDs,guesses,trained_AI_model,name_trained_AI_model,nb_points_traj,stationary_error,stationary_error_timestap,average_smallest_motive_lenght,zero_amount,speedcap,distance_charac_robot,model_name,fit_to_ram,training,trajectory_dataset_name, circles,lines,interpolations, realistic_trajs, selection_erreures_moteur, len_time_series, circle_number, line_number, interpolation_number, realistic_trajs_number,jvalues)
%_____
%Cuts up dataset generations into datasets that will fit inside of 16gigs
%of RAM to avoid the error out of RAM interruption (chunks of 500 trajs
%approximatively)
%
%   n:
%_____
    ...
    quotient_resultat_cercle = floor(circle_number / 500);
    reste_resultat_cercle = rem(circle_number, 500);
    if reste_resultat_cercle == 0
        reste_resultat_cercle=[];
    end
   
    if quotient_resultat_cercle==0
        repeatedElements =[];
    else
        repeatedElements = repmat(500, 1, quotient_resultat_cercle);
    end
    
    generation_list=[generation_list,repeatedElements,reste_resultat_cercle];

    quotient_resultat_ligne = floor(line_number / 500);
    reste_resultat_ligne = rem(line_number, 500);
    if reste_resultat_ligne == 0
        reste_resultat_ligne=[];
    end
    
    if quotient_resultat_ligne==0
        repeatedElements =[];
    else
        repeatedElements = repmat(500, 1, quotient_resultat_ligne);
    end
    
    generation_list=[generation_list,repeatedElements,reste_resultat_ligne];


    quotient_resultat_interpolation = floor(interpolation_number / 500);
    reste_resultat_interpolation = rem(interpolation_number, 500);
    if reste_resultat_interpolation == 0
        reste_resultat_interpolation=[];
    end
    if quotient_resultat_interpolation==0
        repeatedElements =[];
    else
        repeatedElements = repmat(500, 1, quotient_resultat_interpolation);
    end
    
    generation_list=[generation_list,repeatedElements,reste_resultat_interpolation];

    quotient_resultat_realistic_trajs = floor(realistic_trajs_number / 500);
    reste_resultat_realistic_trajs = rem(realistic_trajs_number, 500);
    if reste_resultat_realistic_trajs == 0
        reste_resultat_realistic_trajs=[];
    end

    if quotient_resultat_realistic_trajs==0
        repeatedElements =[];
    else
        repeatedElements = repmat(500, 1, quotient_resultat_realistic_trajs);
    end

    repeatedElements = repmat(500, 1, quotient_resultat_realistic_trajs);
    generation_list=[generation_list,repeatedElements,reste_resultat_realistic_trajs];
    
    total_number_of_generations= quotient_resultat_cercle+quotient_resultat_ligne+quotient_resultat_interpolation+quotient_resultat_realistic_trajs+4;
    fprintf('Le nombre de générations avant la fusion  finale sera : %d points.\n',total_number_of_generations);
    fprintf('Number of trajectories in each rescpective sub-generation dataset due to fit_to_ram: [%s%d]\n', sprintf('%d,', generation_list(1:end-1)),generation_list(end));
    % Initialize the merged cell
    merged_cell = {};
    %%pour l'instant ce code ne marche qu'avec des realistic trajectories
    %%il faudrit rajouter des étapes de découpages et de processing sinon
    for i = 1:total_number_of_generations
        % Creating trajectory dataset name with a prefix
        trajectory_dataset_name_i = ['T', num2str(i), '_', trajectory_dataset_name];
        
        % Call to processing_simulation function
        number_of_elem= generation_list(i);
        %potentiellement forcer simulating a 1 ? qui de training?
        processing_simulation(1, 'training',0, 'trajectory_dataset_name', trajectory_dataset_name_i,'circles',circles, 'circle_number', circle_number,'realistic_trajs', 1, 'realistic_trajs_number',number_of_elem, 'selection_erreures_moteur', selection_erreures_moteur,'nb_points_traj',len_time_series,"trajectory_dataset_name", trajectory_dataset_name,"circles", circles,"circle_number", circle_number,"lines", lines,"interpolations", interpolations,"realistic_trajs", realistic_trajs,"selection_erreures_moteur", selection_erreures_moteur,"fit_to_ram", fit_to_ram,"model_name", model_name,"distance_charac_robot", distance_charac_robot,"speedcap", speedcap,"zero_amount", zero_amount,"average_smallest_motive_lenght", average_smallest_motive_lenght,"joint1_damping", joint1_damping,"joint2_damping", joint2_damping,"damp_pince", damp_pince,"mdl", mdl,"base", base,"follower", follower,"targetIDs", targetIDs,"outputIDs", outputIDs,"guessesIDs", guessesIDs,"guesses", guesses,"trained_AI_model", trained_AI_model,"name_trained_AI_model", name_trained_AI_model,"nb_points_traj", nb_points_traj,"stationary_error", stationary_error,"stationary_error_timestap", stationary_error_timestap,'jvalues',jvalues);
        clear cellArray;
        % Load data into a cell array
        cell = load(trajectory_dataset_name_i, '-mat');
        disp('Successfully loaded so probably well saved')

        % Append loaded data to the merged cell
        fields = fieldnames(cell);
        firstFieldName = fields{1};
        merged_cell{i} = cell.(firstFieldName);
        clear cell;
    end
    
    %Save merged dataset

    save(trajectory_dataset_name, 'cellArray', '-v7.3');
    
    %Train with merfed dataset
    processing_simulation(0,'training',1,'trajectory_dataset_name',trajectory_dataset_name,'selection_erreures_moteur',selection_erreures_moteur,'name_trained_AI_model',name_trained_AI_model)
