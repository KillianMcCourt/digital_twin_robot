...
simOut = sim(model_name);
        ...
        %lieu of comparing motor angular positions.
            ...
            %plot_comparison(x_target, x_real, "XGRAPH_REAL")
            %plot_comparison(x_target, x, "XGRAPH")

        
            mse = mean((comparison_matrix_real - comparison_matrix).^2, 'all');
            disp("MSE for ki=")
            disp(ki)
            disp(mse)
            results(ki) = mse;
            true_labels = [true_labels; mod(ki, 13)]; % True labels
            predicted_labels = [predicted_labels; ki]; % Predicted labels (adjust as needed)
end
     end
     [~, indexOfMin] = min(results);
     disp(indexOfMin)
     if indexOfMin == mod(k,13)
